      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-100 transform transition-transform duration-200
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-slate-100">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <div className="text-sm font-semibold text-slate-900">{userProfile?.name}</div>
              <div className="text-xs text-slate-400">{userProfile?.societyName}</div>
            </div>
          </div>
                 ...
                 entries.slice(0, 5).map((e) => (
                  <div key={e.id} className="flex items-center gap-3 py-2 border-b border-slate-50 last:border-0">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-slate-400">{e.purpose} · Flat {e.flatNumber}</div>
                    </div>
              ...
              {entriesLoading ? <div className="text-center py-8 text-slate-400">Loading...</div> :
               ...
               <div className="space-y-2">{entries.map((e) => (
                <div key={e.id} className="flex items-center gap-3 py-3 border-b border-slate-50 last:border-0">
                  ...
                  <div className="flex-1">
                    <div className="font-semibold text-slate-800 text-sm">{e.visitorName}</div>
                    <div className="text-xs text-slate-400">{e.purpose} · Flat {e.flatNumber} · {e.createdAt}</div>
                  </div>
                  <span className={`badge text-xs ${statusColor(e.status)}`}>{e.status}</span>
                </div>
               ))}</div>}
            ...
            <div className="card">
              ...
              {entries.filter(e => e.status === "pending").length === 0 ? (
                ...
                <div key={e.id} className="flex items-center gap-3 py-3 border-b border-slate-50 last:border-0">
                  ...
                  <div className="flex-1">
                    <div className="font-semibold text-slate-800 text-sm">{e.visitorName}</div>
                    <div className="text-xs text-slate-400">{e.purpose} · Flat {e.flatNumber} · Added by {e.addedBy}</div>
                  </div>
          ...
          {activeTab === "facilities" && (
            <div className="space-y-4">
              ...
              {showForm && (
                <div className="card border-2 border-primary-100">
                  <form onSubmit={handleAddFacility} className="space-y-3">
                    ...
                    <input type="number" placeholder="Capacity (max people) *" required
                      value={facilityForm.capacity} onChange={(e) => setFacilityForm({ ...facilityForm, capacity: e.target.value })}
                    ...
                    <input type="number" placeholder="Price per slot (₹) — leave blank if free"
                      value={facilityForm.price} onChange={(e) => setFacilityForm({ ...facilityForm, price: e.target.value })}
                      className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary-400 text-slate-800 placeholder-slate-400" />
                    <input type="text" placeholder="Description (optional)"
                      value={facilityForm.description} onChange={(e) => setFacilityForm({ ...facilityForm, description: e.target.value })}
                      className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary-400 text-slate-800 placeholder-slate-400" />
              ...
              {facilitiesLoading ? <div className="card text-center py-8 text-slate-400">Loading...</div> :
               facilities.length === 0 ? (
                <div className="card text-center py-12"><div className="text-4xl mb-2">🏊</div><p className="text-slate-500">No facilities added yet.</p></div>
              ) : (
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-slate-500">Capacity: {f.capacity}</div>
                        ...
                        <button onClick={() => deleteFacility(f.id)} className="p-2 hover:bg-red-50 rounded-lg text-slate-400 hover:text-red-500">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                      <div className="mt-3 pt-3 border-t border-slate-100 text-xs text-slate-400">
                        Bookings today: {bookings.filter(b => b.facilityId === f.id && b.date === new Date().toISOString().split("T")[0]).length}
                      </div>
                    </div>
                  ))}
                </div>
