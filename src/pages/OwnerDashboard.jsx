      ...
      <main className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        ...
        {activeTab === "overview" && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: "Add Visitor",     tab: "visitors",   color: "bg-blue-50 hover:bg-blue-100 text-blue-700",     icon: ClipboardList },
                { label: "Add Vehicle",     tab: "vehicles",   color: "bg-orange-50 hover:bg-orange-100 text-orange-700", icon: Car },
                { label: "Book Facility",   tab: "bookings",   color: "bg-green-50 hover:bg-green-100 text-green-700",   icon: Calendar },
                { label: "Complaints",      tab: "complaints", color: "bg-red-50 hover:bg-red-100 text-red-700",         icon: AlertCircle },
                { label: "Chat Members",    tab: "members",    color: "bg-purple-50 hover:bg-purple-100 text-purple-700", icon: MessageCircle },
                { label: "Domestic Help",   tab: "domestic",   color: "bg-pink-50 hover:bg-pink-100 text-pink-700",      icon: HeartHandshake },
                ...(isOwner ? [{ label: "Invite Tenant", tab: "tenants", color: "bg-teal-50 hover:bg-teal-100 text-teal-700", icon: Users }] : []),
              ].map((a) => {
                const Icon = a.icon;
                return (
                  <button key={a.label} onClick={() => { setActiveTab(a.tab); setShowForm(false); }}
                    className={`flex items-center gap-3 p-4 rounded-xl transition-all text-left card ${a.color}`}>
              ...
              <div className="card border-2 border-primary-100">
                <form onSubmit={handleAddVisitor} className="space-y-3">
                  ...
                  <input type="tel" placeholder="Visitor Phone" value={visitorForm.visitorPhone}
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary-400 text-slate-800 placeholder-slate-400" />
                  <select required value={visitorForm.purpose}
                ...
                <div key={e.id} className="card">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-slate-500">{e.purpose}</div>
                    </div>
        ...
        {/* TENANTS - Owner sees InviteTab, Tenant sees message */}
        {activeTab === "tenants" && isOwner && <InviteTab inviteRole="tenant" />}
        {activeTab === "tenants" && isTenant && (
          <div className="card text-center py-12">
            <Users className="w-12 h-12 text-slate-200 mx-auto mb-3" />
            <p className="text-slate-500">Tenant management is handled by your flat owner.</p>
          </div>
        )}
        ...
        {activeTab === "bookings" && (
          <div className="space-y-4">
            ...
            {facilitiesLoading ? (
              <div className="card text-center py-8 text-slate-400">Loading facilities...</div>
            ) : facilities.length === 0 ? (
              <div className="card text-center py-12">
                <Building2 className="w-12 h-12 text-slate-200 mx-auto mb-2" />
                <p className="text-slate-500">No facilities added by admin yet.</p>
              </div>
            ) : (
              <div className="space-y-4">
                <div className="card">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {facilities.map((f) => (
                      <button key={f.id} onClick={() => { setSelectedFacility(f); setBookingSlot(""); }}
                        ...
                        {f.price && <div className="text-xs text-primary-600 font-semibold mt-0.5">Rs.{f.price}/slot</div>}
                      </button>
                    ))}
                  </div>
                </div>
                {selectedFacility && (
                  <div className="card border-2 border-primary-100">
                    <h4 className="font-display font-semibold text-slate-900 mb-4">Book {selectedFacility.name}</h4>
                    <div className="space-y-4">
                      <div>
